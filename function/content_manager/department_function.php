...
// START >> ADD FUNCTION OF DEPARTMENT
if (isset($_POST['add_department'])) {
  ...
  if (!empty($_FILES["dm_image"]["name"])) {
      ...
      move_uploaded_file($_FILES["dm_image"]["tmp_name"], $image_path);
  }
  ...
  if ($conn->query($query) === TRUE) {
      ...
      // Redirect to the department details page
      header("Location: Manage_Department");
      exit();
  } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_faculty']) && isset($_POST['fm_id'])) {
    ...
    if (!$faculty) {
        ...
        header("Location: ../pages/content_manager/view_department?department_id=$deptId");
        exit();
    }
}
