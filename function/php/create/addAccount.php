...
if (isset($_POST['addAccountBtn'])) {
    ...
    try {
        ...
        $stmt_user = $conn->prepare("INSERT INTO user_account (username, `password`, email, `image`, user_type, account_status) VALUES (?, ?, ?, ?, ?, ?)");
        ...
        if ($stmt_user->execute()) {
            ...
            if (!$stmt_ap->execute()) {
                $conn->rollback();
                $_SESSION['toastMsg'] = "Error: Second Query Failed.";
                $_SESSION['toastType'] = "toast-error";
                header("Location: /ukt/pages/adminukt/pending_account.php");
                exit;
            }
            ...
            if (!$stmt_log->execute()) {
                $conn->rollback();
                $_SESSION['toastMsg'] = "Error: Third Query Failed.";
                $_SESSION['toastType'] = "toast-error";
                header("Location: /ukt/pages/adminukt/pending_account.php");
                exit;
            }
            ...
            $conn->rollback();
            $_SESSION['toastMsg'] = "Error: First Query Failed.";
            $_SESSION['toastType'] = "toast-error";
            header("Location: /ukt/pages/adminukt/pending_account.php");
            exit;
        }
