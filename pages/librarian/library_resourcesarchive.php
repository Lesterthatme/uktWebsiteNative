        ...
        <div class="card border-0">
          ...
          <div class="doc-tabs-container mt-3">
              <ul class="doc-tabs d-flex list-unstyled">
                ...
                <li class="me-3">
                  <a class="doc-link active" href="library_resourcesarchive">Library Resources Archive</a>
                </li>
                <li class="me-3">
            ...
            <!-- <h5 class="card-title fs-6 mb-2 mb-md-0">Library Resources Archive</h5> -->
            <!-- View Archive start -->
            <div class="log_container mt-3">
              ...
              $query = "SELECT 
                  ...
                  record_id, 
                  archived_at, 
                  JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.resource_title')) AS resource_title,
                  JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.resource_ISBN')) AS resource_ISBN,
                  JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.resource_author')) AS resource_author,
                  JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.publication_year')) AS publication_year,
                  JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.resource_type')) AS resource_type
              FROM library_archive
              WHERE original_table = 'library_resources'
                  ...
                  <tbody>
                    ...
                    if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                        ...
                        $resource_ISBN = htmlspecialchars($row['resource_ISBN']);
                        $resource_author = htmlspecialchars($row['resource_author']);
                        $publication_year = htmlspecialchars($row['publication_year']);
                        $resource_type = ucfirst(htmlspecialchars($row['resource_type']));
                        $archived_at = date("F j, Y, g:i A", strtotime($row['archived_at']));

                        echo "<tr>
                        <td>$resource_title</td>
                        <td>$resource_ISBN</td>
                        <td>$resource_author</td>
                          ...
                          <div class='dropdown'>
                            ...
                            <ul class='dropdown-menu'>
                              <li>
                                ...
                                href='function/restore_function.php?restoreresources_id=" . $row['record_id'] . "' 
