...
<body class="bg-light">
 ...
 <!-- include side bar start -->
        ...
        <div class="card border-0 pb-3">
          <div class="card-body">
            ...
            <div class="log_container mt-3">
              ...
              record_id, 
              archived_at, 
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.poster_image')) AS poster_image,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.poster_status')) AS poster_status,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.poster_date')) AS poster_date
            FROM university_archive
            WHERE original_table = 'page_poster'
            ORDER BY archived_at DESC";
              ...
              <div class="table-container">
                <table class="table table-hover text-center align-middle" id="activityTable">
                  <thead>
                    <tr>
                      <th class="text-start">Poster Image</th>
                  ...
                  <tbody>
                    ...
                    if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                 <td class='text-start'> 
                                  <img src='../../assets/uploads/poster/" . htmlspecialchars($row['poster_image']) . "' 
                                  alt='news Image' class='news Image' style='width: 90px; height: 90px; object-fit: cover; border-radius: 10px;'>
                                </td>
                                 <td class='text-start'>" . htmlspecialchars($row['poster_status']) . "</td>
                                 <td class='text-start'>" . date("F j, Y", strtotime($row['poster_date'])) . "</td>
                                 <td>
                                    <div class='dropdown'>
                                        ...
                                        <ul class='dropdown-menu'>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/content_manager/restorearchived_function.php?restoreposter_id=" . $row['record_id'] . "' 
                                                    onclick='return confirm(\"Do you want to restore this poster?\")' 
                                                    data-bs-toggle='tooltip' 
                                                    data-bs-placement='right' 
                                                    title='Click here to restore the poster'>
                                                    <i class='ri-reset-left-line'></i> Restore
                                                </a>
                                            </li>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/content_manager/restorearchived_function.php?deleteposter_id=" . $row['record_id'] . "' 
                                                    onclick='return confirm(\"Do you want to delete this poster?\")' 
                                                    data-bs-toggle='tooltip' 
                                                    data-bs-placement='right' 
                                                    title='Click here to permanent delete the poster'>
  ...
  <script src="../../assets/bootstrap/js/activeLink.js?v=1.11"></script>
