        ...
        <div class="card border-0 pb-3">
          <div class="card-body">
            ...
            <div class="log_container mt-3">
              ...
              record_id, 
              archived_at, 
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.up_image')) AS up_image,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.up_name')) AS up_name,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.up_date')) AS up_date,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.up_link')) AS up_link
          FROM university_archive
          WHERE original_table = 'university_partnership'
                  ...
                  <tbody>
                    ...
                    if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                <td> 
                                  <img src='../../assets/uploads/partnership/" . htmlspecialchars($row['up_image']) . "' 
                                  alt='University Logo' class='university-logo' style='width: 50px; height: 50px; object-fit: cover; border-radius: 50px;'>
                                </td>
                                <td>" . htmlspecialchars($row['up_name']) . "</td>
                                <td>" . date("F j, Y", strtotime($row['up_date'])) . "</td>
                                <td>
                                    <a href='" . (strpos($row['up_link'], 'http') === 0 ? htmlspecialchars($row['up_link']) : 'https://' . htmlspecialchars($row['up_link'])) . "' 
                                      target='_blank'>" . htmlspecialchars($row['up_link']) . "</a>
                                </td>
                                <td>
                                    <div class='dropdown'>
                                        ...
                                        <ul class='dropdown-menu'>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/content_manager/restorearchived_function.php?restorepartnership_id=" . $row['record_id'] . "' 
                                                     onclick='return confirm(\"Do you want to restore this Partnership?\")' 
                                                data-bs-toggle='tooltip' 
                                                data-bs-placement='right' 
                                                title='Click here to restore the University Partnership'>
                                            ...
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/content_manager/restorearchived_function.php?deletepartnership_id=" . $row['record_id'] . "' 
                                                   onclick='return confirm(\"Do you want to delete this Partnership?\")' 
                                                data-bs-toggle='tooltip' 
                                                data-bs-placement='right' 
                                                title='Click here to permanent delete the partnership'>
                      ...
                      echo "<tr><td colspan='5'>No archived university partnerships available</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
