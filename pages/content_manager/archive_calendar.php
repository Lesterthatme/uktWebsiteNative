        ...
        <div class="card border-0 pb-3">
          <div class="card-body">
            ...
            <div class="log_container mt-3">
              ...
              record_id, 
              archived_at, 
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.uc_title')) AS uc_title,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.uc_month')) AS uc_month,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.uc_day')) AS uc_day,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.uc_description')) AS uc_description
            FROM university_archive
            WHERE original_table = 'university_calendar'
            ORDER BY archived_at DESC";
              ...
              <div class="table-container">
                <table class="table table-hover text-center align-middle" id="activityTable">
                  <thead>
                    <tr>
                      <th class="text-start">Event Title</th>
                  ...
                  <tbody>
                    ...
                    if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                 <td class='text-start'>" . htmlspecialchars($row['uc_title']) . "</td>
                                 <td class='text-start'>" . htmlspecialchars($row['uc_month']) . "</td>
                                 <td class='text-start'>" . htmlspecialchars($row['uc_day']) . "</td>
                                 <td>
                                    <div class='dropdown'>
                                        ...
                                        <ul class='dropdown-menu'>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/content_manager/restorearchived_function.php?restoreevent_id=" . $row['record_id'] . "' 
                                                    onclick='return confirm(\"Do you want to restore this event?\")' 
                                                    data-bs-toggle='tooltip' 
                                                    data-bs-placement='right' 
                                                    title='Click here to restore the University Calendar Event'>
                                                     <i class='ri-reset-left-line'></i> Restore
                                                </a>
                                            </li>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/content_manager/restorearchived_function.php?deleteevent_id=" . $row['record_id'] . "' 
                                                    onclick='return confirm(\"Do you want to delete this event?\")' 
                                                    data-bs-toggle='tooltip' 
                                                    data-bs-placement='right' 
                                                    title='Click here to permanent delete the event'>
                      ...
                      echo "<tr><td colspan='5'>No archived university calendar found</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
