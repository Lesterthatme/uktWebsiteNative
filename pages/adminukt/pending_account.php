...
<body class="bg-light">
          ...
          <div class="card-body">
            ...
            <div class="doc-tabs-container mt-3">
              <ul class="doc-tabs d-flex list-unstyled">
                <li class="me-3">
                  <a class="doc-link active" href="pending_account">Pending</a>
                </li>
                <li class="me-3">
                      ...
                      <?php while ($row = $result->fetch_assoc()): ?>
                        <tr>
                          ...
                          <td class="text-center">
                            ...
                            $badgeClass = ($status === 'pending') ? 'status-badge-warning' : 'status-badge';
                            ?>
                              ...
                              <ul class="dropdown-menu">
                                <li>
                                  <a class="dropdown-item text-dark view-profile" href="#" data-bs-toggle="modal"
                                    data-bs-target="#profileModal<?= $row['user_id'] ?>"
                                    data-image="<?= htmlspecialchars($row['image']) ?>"
                                    data-fullname="<?= htmlspecialchars($row['ap_firstname'] . ' ' . ($row['ap_mi'] ? $row['ap_mi'] . ' ' : '') . $row['ap_lastname']) ?>"
                        ...
                        <div class="modal fade" id="profileModal<?= $row['user_id'] ?>" tabindex="-1"
                          aria-labelledby="profileModalLabel<?= $row['user_id'] ?>" aria-hidden="true">
                          <div class="modal-dialog">
                            <div class="modal-content">
                              <div class="modal-header">
                                <h5 class="modal-title fw-bold text-muted" id="profileModalLabel<?= $row['user_id'] ?>">
                                  Profile Details</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                              </div>
                        ...
                        <!-- End Modal for Each User -->

                      <?php endwhile; ?>
                    <?php else: ?>
                      <tr>
                        <td colspan="5" class="text-center text-warning">No pending accounts found.</td>
                      </tr>
                    <?php endif; ?>
                  </tbody>
                </table>
  ...
  <script>
    function previewImage(event) {
      const fileInput = event.target; // Correctly reference the file input element
      const profilePic = document.getElementById("profile-pic"); // Correctly reference the image element
