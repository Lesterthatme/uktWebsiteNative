...
<body class="bg-light">
        ...
        <div class="card border-0 pb-3">
          <div class="card-body">
            ...
            <div class="log_container mt-3">
              ...
              record_id, 
              archived_at, 
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.requirement_title')) AS requirement_title,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.date_added')) AS date_added,
              JSON_UNQUOTE(JSON_EXTRACT(archive_description, '$.status')) AS status
            FROM university_archive
            WHERE original_table = 'admission_requirement'
                ...
                <table class="table table-hover text-center align-middle" id="activityTable">
                  <thead>
                    <tr>
                      ...
                      <th class="text-start">Requirement Title</th>
                  ...
                  <tbody>
                    ...
                    if ($result->num_rows > 0) {
                      while ($row = $result->fetch_assoc()) {
                        echo "<tr>
                                <td class='text-start'>" . date("F j, Y", strtotime($row['date_added'])) . "</td>
                                <td class='text-start'>" . htmlspecialchars($row['requirement_title']) . "</td>
                                <td class='text-start'>" . htmlspecialchars($row['status']) . "</td>
                                        ...
                                        <ul class='dropdown-menu'>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/restorearchived_function.php?restorerequirement_id=" . $row['record_id'] . "' 
                                                    onclick='return confirm(\"Do you want to restore this admission requirement?\")' 
                                                    data-bs-toggle='tooltip' 
                                                    data-bs-placement='right' 
                                                    title='Click here to restore the admission requirement'>
                                                    <i class='ri-reset-left-line'></i> Restore
                                                </a>
                                            </li>
                                            <li>
                                                <a class='dropdown-item text-dark' 
                                                    href='../../function/restorearchived_function.php?deleteadrequirement_id=" . $row['record_id'] . "' 
                                                    onclick='return confirm(\"Do you want to delete this admission requirement?\")' 
                                                    data-bs-toggle='tooltip' 
                                                    data-bs-placement='right' 
                                                    title='Click here to permanent delete the admission requirement'>
                      ...
                      echo "<tr><td colspan='5'>No archived admission requirements found</td></tr>";
                    }
                    ?>
                  </tbody>
                </table>
  ...
  <script src="../../assets/bootstrap/js/activeLink.js?v=1.12"></script>
